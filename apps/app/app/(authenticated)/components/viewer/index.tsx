import type { formSchema as chatSchema } from './chat/chat-input';
...
export function Viewer({ botId, isLoading, meeting: data }: ViewerProps) {
  ...
  const [editor, setEditor] = React.useState<JSONContent | undefined>(
    undefined
  );
  ...
  const { editor: editorDB, isLoading: isEditorLoading } = useEditor({
    meetingId: data.id,
  });

  const { messages: chatMessages, isLoading: isChatLoading } = useChat({
    meetingId: data.id,
  });

  const { trigger: leaveMeeting, isMutating: isLeavingMeeting } =
    useSWRMutation(
      ['leaveMeeting', botId, baasApiKey],
      ([key, botId, baasApiKey]) =>
        leaveMeetingQuery({ botId, apiKey: baasApiKey! })
    );
  ...
  return (
    <div className="min-h-svh">
      <div className="w-full">
        <header className="sticky top-0 flex h-16 shrink-0 items-center justify-between gap-2 bg-background px-4">
          ...
          <div className="flex flex-grow items-center justify-center text-center">
            <h1 className="font-semibold text-xl">{localMeetingName}</h1>
            <Button
              variant="ghost"
              size="icon"
              className="ml-2"
          ...
          <label htmlFor="show-editor-chat" className='font-medium text-sm'>
            Show Editor & Chat
          </label>
          <Switch
            id="show-editor-chat"
      ...
      <div
        className={cn(
          'flex justify-center',
          showEditorChat ? 'w-full' : 'mx-auto max-w-3xl'
        )}
      >
        <ResizablePanelGroup
